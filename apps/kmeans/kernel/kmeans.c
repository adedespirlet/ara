...
void assignPointsToClusters(const int64_t *points, const int64_t *centers, int64_t *clusters){
    ...
    for (; avl > 0; avl -= vl) {
        ...
        asm volatile("vmerge.vvm v4, v8, v4, v0.t") ;//replace elements in v4 by v8 if mask is set
        asm volatile("vmslt.vv v0, v12, v4");    //mask vector set if elements in v8 are smaller than v4
        asm volatile("vmerge.vim v16,v16,2,v0.t");//set cluster number to 1 if mask is set
        //store resuls back
